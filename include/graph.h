...
public:
    ...
    std::vector<tuple<double,double,int>> acc_xy_quadrangle_graph_cn_and_edges_density();
private:
    vector<vector<nei_jc>> _CN();
    vector<vector<nei_jc>> _JC();
    double _acc_xy_quadrangle_graph_i_j_ave_cn(int i, int j);
    double _acc_xy_quadrangle_graph_i_j_edges_density(int i, int j);
};
